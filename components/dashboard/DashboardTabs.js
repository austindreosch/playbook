      ...
      {availableTabs.map(({ id, label, enabled }) => {
        ...
        return (
          <button
            ...
            disabled={!enabled}
            className={`
              flex-1 flex items-center justify-center gap-2 text-label-sm tracking-wider leading-relaxed uppercase focus:outline-none select-none transition-colors
                          ${currentTab === id
                ? 'h-full bg-pb_paperwhite text-black font-semibold  border border-pb_darkgray border-b-0 rounded-t-lg mt-1'
                : 'h-full text-white hover:bg-gray-450 font-semibold rounded-md mt-[5px] hover:mt-2 hover:mb-[3px]'}
            disabled:opacity-70 disabled:cursor-not-allowed
          `}
            onClick={() => handleClick(id)}
          >
            {/* Icon */}
            {IconComponent && <IconComponent className="hw-icon-xs" />}
            {/* ---- DISPLAY DATA ---- */}
            {/* Display tab label */}
            {label}
          </button>
