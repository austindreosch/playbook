            ...
            {performanceData.metrics.map((metric, index) => (
              <div key={index} className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <SigmaSquare className="w-3 h-3 text-pb_textgray" />
          ...
          {performanceData.metrics[0].changes.length > 0 && (
            <div className="mt-2 text-right">
              {performanceData.metrics[0].changes.map((change, index) => (
                <div key={index} className="text-2xs text-pb_textgray">
                  {change.label}
                </div>
              ))}
            </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-4 gap-1">
            {impactStats.map((stat, index) => (
              <div 
                key={index}
                className={`text-center py-2 px-1 text-2xs font-medium rounded ${getImpactColor(stat)}`}
              >
                {stat}
              </div>
            ))}
