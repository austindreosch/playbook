        ...
        for (const sport of SUPPORTED_SPORTS) {
            ...
            try {
                ...
                for (const playerEntry of msfPlayerEntries) {
                    ...
                    try {
                        // 4. Check if player exists using the core data source key for ID
                        const existingPlayer = await playersCollection.findOne({ [`${CORE_DATA_SOURCE_KEY}.id`]: msfId });
                        const now = new Date();

                        if (existingPlayer) {
                            // 5. Update Existing Player
                            // Check if data actually changed
                            // Use CORE_DATA_SOURCE_KEY for checking nested fields
                            if (existingPlayer.primaryFirstName !== firstName ||
                                existingPlayer.primaryLastName !== lastName ||
                                existingPlayer[CORE_DATA_SOURCE_KEY]?.name !== primaryName ||
                                existingPlayer[CORE_DATA_SOURCE_KEY]?.lastVerified?.toISOString() !== sourceTimestamp.toISOString())
                            {
                                const updateFields = {
                                    primaryFirstName: firstName,
                                    primaryLastName: lastName,
                                    primaryName: primaryName,
                                    // Use dot notation with CORE_DATA_SOURCE_KEY
                                ...
                                // Ensure primaryName is in variants even if no other update needed
                            ...
                            const newPlayerData = {
                                ...
                                primaryLastName: lastName,
                                primaryName: primaryName,
                                nameVariants: [primaryName],
                                sport: sport,
                                position: position,
                                teamName: teamName,
                                teamId: teamId,
                                // Use CORE_DATA_SOURCE_KEY for the nested object
                                [CORE_DATA_SOURCE_KEY]: { id: msfId, name: primaryName, lastVerified: sourceTimestamp },
                                // Initialize fantasy platforms using FANTASY_PLATFORMS from config (can be done here or later)
                                // Example: ...Object.fromEntries(FANTASY_PLATFORMS.map(key => [key, {id: null, name: null, lastVerified: null}]))
                                espn: { id: null, name: null, lastVerified: null }, // Keeping explicit for now
                        ...
                        results.errors.push(`DB error for ${primaryName} (${CORE_DATA_SOURCE_KEY} ID: ${msfId}): ${dbError.message}`);
                    }
