...
async function matchPlayer(nameToMatch, sport, playersCollection, fuseInstance) {
    ...
    // 3. Try fuzzy match using Fuse.js instance (if not found by exact matches)
    if (!matchedPlayerDoc && fuseInstance) {
        ...
        if (fuseResults.length > 0 && fuseResults[0].score != null && fuseResults[0].score <= fuseOptions.threshold) {
            const fuzzedItem = fuseResults[0].item;
            // Fetch coreId again if it wasn't part of the initial Fuse item projection (it should be now)
            const coreId = fuzzedItem[CORE_DATA_SOURCE_KEY]?.id ||
                           (await playersCollection.findOne({ _id: fuzzedItem._id }, { projection: { [`${CORE_DATA_SOURCE_KEY}.id`]: 1 } }))?.[CORE_DATA_SOURCE_KEY]?.id;
            matchedPlayerDoc = { _id: fuzzedItem._id, coreId: coreId, nameVariants: fuzzedItem.nameVariants || [], primaryName: fuzzedItem.primaryName };
            matchType = 'fuzzy';
            // console.log(`  Fuzzy match for: '${nameToMatch}' -> '${matchedPlayerDoc.primaryName}' (Score: ${fuseResults[0].score.toFixed(3)}) (Core ID: ${matchedPlayerDoc.coreId})`);
        }
    }
