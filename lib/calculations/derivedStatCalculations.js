...
export function calculateNflPpsHalfPpr(stats) {
    // requiredStats: ['offenseSnaps', 'GP', 'passYds', 'passTD', 'interceptions', 'rushYds', 'rushTD', 'rec', 'recYds', 'recTD', 'fumLost']
    const snaps = stats.offenseSnaps;
    if (typeof snaps !== 'number' || snaps === 0) return 0;
    const basePoints = calculateNflBaseFantasyPoints(stats);
    const recPoints = (stats.rec || 0) * 0.5;
    const totalPoints = basePoints + recPoints;
    return totalPoints / snaps;
}

export function calculateNflPpsFullPpr(stats) {
     // requiredStats: ['offenseSnaps', 'GP', 'passYds', 'passTD', 'interceptions', 'rushYds', 'rushTD', 'rec', 'recYds', 'recTD', 'fumLost']
     const snaps = stats.offenseSnaps;
     if (typeof snaps !== 'number' || snaps === 0) return 0;
     const basePoints = calculateNflBaseFantasyPoints(stats);
...
export function calculateNflYardShare(playerStats, context) {
    ...
    if (!teamTotals) {
        // console.warn('[calculateNflYardShare] Missing teamTotals in context.'); // Commented out warning
        return null; // Cannot calculate without team totals
    }
    ...
    // Assuming teamTotals has similar keys - adjust path if needed
    const teamPassYds = teamTotals.passYds || 0;
    const teamRushYds = teamTotals.rushYds || 0;
    const teamRecYds = teamTotals.recYds || 0; // ADDED: Include team receiving yards
    const teamTotalYds = teamPassYds + teamRushYds + teamRecYds; // UPDATED: to include teamRecYds

    if (teamTotalYds === 0) return 0; // Avoid division by zero

    return (playerTotalYds / teamTotalYds) * 100; // Convert to percentage
}
...
export function calculateNflProductionShare(playerStats, context) {
    ...
    // --- ADD LOGGING for PR% inputs ---
    // const playerFullNameForLog = `${playerStats.firstName || ''} ${playerStats.lastName || ''}`.trim(); // Need info passed for name
    // Limit logging to specific players if needed
    // if (playerFullNameForLog === 'Joe Flacco' || playerFullNameForLog === 'Kyle Juszczyk') {
        // console.log(`[calculateNflProductionShare] Inputs for PlayerID ${playerStats.playerId}:`, {
        //     playerProd: { passComp: playerPassComp, rushAtt: playerRushAtt, rec: playerRec, total: playerTotalProd },
        //     teamProd: { passComp: teamPassComp, rushAtt: teamRushAtt, rec: teamRec, total: teamTotalProd },
        //     calculatedShare: teamTotalProd === 0 ? 0 : (playerTotalProd / teamTotalProd)
        // });
    // }
    // --- END LOGGING ---
...
const calculationImplementations = {
    ...
    calculateNflPpsHalfPpr,
    calculateNflPpsFullPpr,
    calculateNflOpg,
    calculateNflOpe,
...
export function calculateDerivedStats(rawPlayerData, sportConfig, context = {}) {
    ...
    for (const [statAbbrev, definition] of Object.entries(derivedStatDefinitions)) {
        ...
        // +++ ADD LOGGING FOR YD% CONTEXT +++
        if (sportConfig.label === 'NFL' && statAbbrev === 'YD%') {
            console.log(`[calculateDerivedStats NFL YD% Context Check] For stat: ${statAbbrev}, Received context:`, JSON.stringify(context));
            if (!context || !context.teamTotals) {
                console.warn(`[calculateDerivedStats NFL YD% Context Check] teamTotals IS MISSING or context is undefined for YD%`);
            } else {
                console.log(`[calculateDerivedStats NFL YD% Context Check] teamTotals IS PRESENT for YD%. Keys: ${Object.keys(context.teamTotals).join(', ')}`);
            }
        }
        // --- END LOGGING FOR YD% CONTEXT ---
        ...
        if (calculationFunction && typeof calculationFunction === 'function') {
            try {
                // Log inputs for NBA PPG calculation
                if (sportConfig.label === 'NBA' && calculationKey === 'calculateNbaPpg') {
                    const playerIdForLog = _.get(rawPlayerData, infoPathMapping['playerId']);
                    console.log(`[calculateDerivedStats NBA PPG Debug] Player ID: ${playerIdForLog}, For Stat: ${statAbbrev}`);
                    console.log('  Gathered Stats for NBA PPG calc:', JSON.stringify(gatheredStats));
                    // Log the raw stats object from which these were gathered
                    console.log('  Raw player stats object for NBA PPG calc:', JSON.stringify(_.get(rawPlayerData, 'stats'))); 
                }

                // +++ ADD SVH DEBUG LOGGING +++
                if (statAbbrev === 'SVH' && sportConfig.label === 'MLB') {
                    const playerIdForLog = _.get(rawPlayerData, infoPathMapping['playerId']); // Get player ID for context
                    // Log player type for better debugging
                    const playerTypeForLog = isMlbPitcher ? 'Pitcher' : isMlbHitter ? 'Hitter' : 'Unknown MLB Type';
                    console.log(`[calculateDerivedStats SVH Debug] Player ID: ${playerIdForLog}, Type: ${playerTypeForLog}, Stat: ${statAbbrev}`);
                    console.log('  Gathered Stats for SVH calc:', JSON.stringify(gatheredStats));
                }
                // +++ END SVH DEBUG LOGGING +++
