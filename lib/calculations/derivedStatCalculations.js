...
export function calculateDerivedStats(rawPlayerData, sportConfig, context = {}) {
    ...
    for (const [statAbbrev, definition] of Object.entries(derivedStatDefinitions)) {
        ...
        if (calculationFunction && typeof calculationFunction === 'function') {
            try {
                // +++ ADD DEBUG LOG FOR YPS +++
                if (calculationKey === 'calculateNflYardsPerSnap') {
                    console.log(`[Debug YPS] Gathered stats for ${statAbbrev} (${calculationKey}):`, gatheredStats);
                    console.log(`[Debug YPS] Raw player data for offenseSnaps path (${statPathMapping?.offenseSnaps}):`, _.get(rawPlayerData, statPathMapping?.offenseSnaps));
                }
                // +++ END DEBUG LOG +++

                let result;
                // Determines how to call the function based on what it needs (stats, info, context)
                // The specific arguments passed depend on the calculation function's needs, typically defined by requiredStats/requiredInfo
                if (definition.requiredStats && definition.requiredInfo) {
                    result = calculationFunction({ ...gatheredStats, ...gatheredInfo }, context); // For functions needing combined stats & info + context
                } else if (definition.requiredStats) {
                    result = calculationFunction(gatheredStats, context); // For functions needing stats + context (e.g., YPS, YardShare)
                } else if (definition.requiredInfo) {
                    result = calculationFunction(gatheredInfo, context); // For functions needing info + context (e.g., FullName, PreciseAge)
                } else {
                    result = calculationFunction(context); // For functions needing only context (if any)
                }
                
                // Handle numeric results: store null if NaN or Infinity, otherwise store non-numeric directly
                if (typeof result === 'number') {
                    derivedResults[statAbbrev] = (!isNaN(result) && isFinite(result)) ? result : null;
                } else {
                    derivedResults[statAbbrev] = result; // Store non-numeric results (e.g., string, null) directly
                }
