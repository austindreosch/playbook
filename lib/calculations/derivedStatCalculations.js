...
export function calculateDerivedStats(rawPlayerData, sportConfig, context = {}) {
    ...
    for (const [statAbbrev, definition] of Object.entries(derivedStatDefinitions)) {
        ...
        if (calculationFunction && typeof calculationFunction === 'function') {
            try {
                 // --- REMOVED DEBUG LOGGING for Player 6114 (Joe Flacco) PPG 1 PPR Inputs ---
                 // const playerIdForLog = _.get(rawPlayerData, infoPathMapping['playerId']); // Get player ID
                 // if (calculationKey === 'calculateNflPpgFullPpr' && playerIdForLog === 6114) {
                 //     console.log(`[DEBUG Flacco PPG (1 PPR) Inputs @ calculateDerivedStats]:`, JSON.stringify(gatheredStats));
                 // }
                 // --- END REMOVED DEBUG LOGGING ---
