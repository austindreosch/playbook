...
export function calculateMlbSvh(stats) {
    ...
    const sv = typeof stats.SV === 'number' ? stats.SV : 0;
    const hld = typeof stats.HLD === 'number' ? stats.HLD : 0;
    return sv + hld;
}
...
export function calculateNflYardShare(playerStats, context) {
    ...
    if (!teamTotals) {
        console.warn('[calculateNflYardShare] Missing teamTotals in context.');
...
export function calculateNflProductionShare(playerStats, context) {
    ...
    const teamTotals = context?.teamTotals;
     if (!teamTotals) {
         // console.warn('[calculateNflProductionShare] Missing teamTotals in context.'); // Log moved to processNflData
         return null;
     }
...
export function calculateDerivedStats(rawPlayerData, sportConfig, context = {}) {
    ...
    const { statPathMapping, infoPathMapping, derivedStatDefinitions } = sportConfig;
