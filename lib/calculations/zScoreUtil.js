...
export function applyZScores(processedPlayers, statConfigs) {
    ...
    return processedPlayers.map(player => {
        ...
        for (const statKey in statConfigs) {
            ...
            if (originalValue !== undefined && originalValue !== null && stats[statKey]) {
                ...
                const newColors = getZScoreColorsForUtil(
                    zScore,
                    statConfig.impactType,
                    colorRange
                );
                
                // Update the player object with new colors
                setNestedValue(newPlayer, `${statConfig.path}Color`, newColors.bgColor);
                setNestedValue(newPlayer, `${statConfig.path}TextColor`, newColors.textColor);
            }
        }
