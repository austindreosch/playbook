...
export const calculateZScoresWithComparisonPool = (
    ...
    // +++ Log Inputs +++
    console.log('[ZScore Util] Inputs:', {
        playersToScoreCount: playersToScore?.length,
        comparisonPoolCount: comparisonPoolPlayers?.length,
        enabledCategoriesKeys: Object.keys(enabledCategoriesDetails ?? {}),
        // statPathMapping, // Can be large, omit for now
        comparisonRules
    });
    // +++ End Log +++
    ...
    console.log(`[ZScore Util] Starting distribution calculation. Rule type: ${comparisonRules.type}`);
    ...
    // console.log("[ZScore Util] Pre-calculated Distributions:", distributionCache);
    ...
    return playersToScore.map((player, playerIndex) => {
        ...
        // +++ Log final player object before returning +++
        if (playerIndex < 3) { // Log first few
            console.log(`[ZScore Util] Updated player object (index ${playerIndex}):`, updatedPlayer);
        }
        // +++ End Log +++
...
export function applyZScores(players, sportKey) {
    ...
    if (!players || Object.keys(players).length === 0 || !sportConfig || !sportConfig.categories) {
        ...
        return players; // No players or config to process
    }
    ...
    try { // Add error handling around pool creation
        ...
        return players; // Stop processing if pool creation fails
    }
    ...
    try { // Add error handling around summary calculation
        ...
        return players; // Stop processing if summary calculation fails
    }
    ...
    // console.log(`[Z-Score ${sportKey.toUpperCase()}] Calculation time: ${(endTime - startTime).toFixed(2)} ms`);

    return players; // Return the modified map
}
