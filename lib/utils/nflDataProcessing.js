...
export function processNflData(rawSeasonalPlayers = [], rawTeamStatsTotals = [], context = {}) {
    ...
    for (const [playerId, playerStats] of Object.entries(initialPlayerMap)) {
        ...
        // -------------------------------------------------------------------
         ...
         if (teamId) {
             calculationContext.teamTotals = teamTotalsContext[teamId] || null;
         } else {
             // Optional: log missing teamId for debugging
             console.warn(`[processNflData] Missing teamId for player: ${playerInfo.fullName || playerId}`);
         }
        ...
        const teamTotalYds = (teamTotalsContext[teamId]?.passYds || 0) + (teamTotalsContext[teamId]?.rushYds || 0);
        if (teamTotalYds > 0 && (playerTotalYds / teamTotalYds) * 100 > 100) {
            console.warn('[YD% > 100 DEBUG]', {
                playerStats,
                teamTotals: teamTotalsContext[teamId],
                playerTotalYds,
                teamTotalYds,
                ydPercent: (playerTotalYds / teamTotalYds) * 100
            });
        }
    }
