...
export function processNflData(rawSeasonalPlayers = [], rawTeamStatsTotals = []) {
    ...
    for (const [playerId, playerStats] of Object.entries(initialPlayerMap)) {
        ...
        // --- End Refactored Info Mapping ---
         ...
         if (!teamTotalsForPlayer) {
             ...
             console.warn(`[processNflData] teamTotals not found for teamId: ${teamId} (Player: ${playerName}, ID: ${playerId}). Context will be null.`);
         }
         // --- END ADDED LOGGING ---
    ...
    const filteredPlayersMap = {};
     ...
     console.log(`processNflData: Processed ${Object.keys(filteredPlayersMap).length} NFL players.`);
