...
export class YahooIntegration extends PlatformIntegration {
  ...
  async validateCredentials(credentials) {
    if (!credentials || !credentials.accessToken) {
      const error = this.errorHandler.createMissingCredentialsError('accessToken');
      return { 
        valid: false, 
        error: error.message 
      };
    ...
    try {
      // Test credentials by fetching user info with rate limiting
      const response = await this.rateLimiter.execute(
        () => axios.get(`${this.baseUrl}/users;use_login=1`, {
          headers: { 
      ...
      if (response.data) {
        ...
        const validationError = this.errorHandler.createValidationError(
          'credentials', 
          'Invalid response from Yahoo API'
        );
        return { 
          valid: false, 
          error: validationError.message 
      ...
      if (normalizedError.code === 'AUTHENTICATION_FAILED' && credentials.refreshToken) {
        normalizedError.needsRefresh = true;
      }
      ...
      return { 
        valid: false, 
        error: normalizedError.message,
        needsRefresh: normalizedError.needsRefresh 
      };
    }
  }
...
export class ESPNIntegration extends PlatformIntegration {
  ...
  async validateCredentials(credentials) {
    if (!credentials || !credentials.cookies) {
      const error = this.errorHandler.createMissingCredentialsError('cookies');
      return { 
        valid: false, 
        error: error.message 
      };
    ...
    if (!credentials.testLeagueId) {
      const error = this.errorHandler.createValidationError(
        'credentials', 
        'ESPN validation requires a test league ID'
      );
      return { 
        valid: false, 
        error: error.message 
    ...
    try {
      // Test credentials with a simple league settings call using rate limiting
      const response = await this.rateLimiter.execute(
        () => axios.get(
      ...
      if (response.data && response.data.id) {
        ...
        const validationError = this.errorHandler.createValidationError(
          'credentials', 
          'Invalid ESPN cookies or league access denied'
        );
        return { 
          valid: false, 
          error: validationError.message 
