...
export class FantraxIntegration extends PlatformIntegration {
  ...
  detectSportFromLeagueInfo(leagueInfo) {
    // Use scoring category groups as primary detection method (based on API testing)
    const scoringSettings = leagueInfo.scoringSystem?.scoringCategorySettings;
    if (scoringSettings && scoringSettings.length > 0) {
      const groupCodes = scoringSettings.map(setting => setting.group?.code).filter(Boolean);
      
      // MLB: Has both hitting and pitching categories
      if (groupCodes.includes('BASEBALL_HITTING') && groupCodes.includes('BASEBALL_PITCHING')) {
        return 'MLB';
      }
      
      // NBA: Has basketball player categories
      if (groupCodes.includes('BASKETBALL_PLAYER')) {
        return 'NBA';
      }
      
      // NFL: Check for football-specific groups (would need NFL league to test)
      if (groupCodes.some(code => code.includes('FOOTBALL') || code.includes('NFL'))) {
        return 'NFL';
      }
    }
    
    // Fallback to lineup settings check
  ...
  detectScoringTypeFromSettings(leagueInfo) {
    // Categories league: Has scoringCategorySettings array (based on API testing)
    const scoringSettings = leagueInfo.scoringSystem?.scoringCategorySettings;
    if (scoringSettings && Array.isArray(scoringSettings) && scoringSettings.length > 0) {
      return 'Categories';
    }
    
    // Points league: No category settings or empty array
    return 'Points';
  }

  detectMatchupTypeFromSettings(leagueInfo) {
    // H2H league: Has matchups array with matchupList (based on API testing)
    if (leagueInfo.matchups && 
        Array.isArray(leagueInfo.matchups) && 
        leagueInfo.matchups.length > 0 &&
        leagueInfo.matchups[0].matchupList &&
        Array.isArray(leagueInfo.matchups[0].matchupList)) {
      return 'H2H';
    }
    
    // Default fallback - most Fantrax leagues are H2H
    return 'H2H';
  }

  /**
   * Extract categories list from Fantrax API for Categories leagues
   * @param {Object} leagueInfo - Full league info from API
   * @returns {Array} - Array of category short names
   */
  extractCategoriesList(leagueInfo) {
    const scoringSettings = leagueInfo.scoringSystem?.scoringCategorySettings;
    if (!scoringSettings || !Array.isArray(scoringSettings)) {
      return [];
    }

    const categories = [];
    scoringSettings.forEach(setting => {
      if (setting.configs && Array.isArray(setting.configs)) {
        setting.configs.forEach(config => {
          if (config.scoringCategory && config.scoringCategory.shortName) {
            categories.push(config.scoringCategory.shortName);
          }
        });
      }
    });
  ...
  /**
   * Map Fantrax API data to form fields (based on API testing)
   * @param {Object} leagueInfo - Full league info from getLeagueInfo
   * @returns {Object} - Form-ready data with sync tracking
   */
  mapToFormFields(leagueInfo) {
    const sport = this.detectSportFromLeagueInfo(leagueInfo);
    const scoring = this.detectScoringTypeFromSettings(leagueInfo);
    const matchup = this.detectMatchupTypeFromSettings(leagueInfo);
    const categories = this.extractCategoriesList(leagueInfo);
    
    const formData = {
      // Core detectable fields
      sport: sport,
      scoring: scoring,
      matchup: matchup,
      ...
      teamCount: leagueInfo.teamInfo ? Object.keys(leagueInfo.teamInfo).length : 0,
      
      // Draft settings
      draftType: leagueInfo.draftSettings?.draftType === 'snake' ? 'Snake' : 'Auction',
    ...
    // Track which fields were synced from API
    const syncedFields = ['sport', 'scoring', 'matchup', 'leagueName', 'teamCount', 'draftType'];
    if (categories.length > 0) {
      syncedFields.push('categories');
    }
...
export class SleeperIntegration extends PlatformIntegration {
  ...
  detectLeagueType(league) {
    ...
    // Use the official type field if available (based on API testing)
    if (settings.type !== undefined) {
      const typeMap = { 0: 'Redraft', 1: 'Keeper', 2: 'Dynasty' };
      return typeMap[settings.type] || 'Redraft';
    }
  ...
  /**
   * Calculate trade deadline week to date (for display purposes)
   * @param {number} tradeDeadlineWeek - Week number when trades are disabled
   * @returns {Date|null} - Calculated deadline date
   */
  calculateTradeDeadlineDate(tradeDeadlineWeek) {
    if (!tradeDeadlineWeek || tradeDeadlineWeek === 0) return null;
    
    // Approximate calculation - NFL season typically starts early September
    const currentYear = new Date().getFullYear();
    const seasonStart = new Date(currentYear, 8, 10); // September 10th approximation
    const deadlineDate = new Date(seasonStart);
    deadlineDate.setDate(deadlineDate.getDate() + ((tradeDeadlineWeek - 1) * 7));
    
    return deadlineDate;
  }

  /**
   * Map Sleeper API data to form fields (based on API testing) 
   * @param {Object} league - Full league data from Sleeper API
   * @returns {Object} - Form-ready data with sync tracking
   */
  mapToFormFields(league) {
    const settings = league.settings || {};
    const leagueType = this.detectLeagueType(league);
    const tradeDeadlineDate = this.calculateTradeDeadlineDate(settings.trade_deadline);
    
    const formData = {
      // Core fields (always available)
      sport: league.sport ? league.sport.toUpperCase() : 'NFL',
      scoring: 'Points', // Sleeper is always points-based
      matchup: 'H2H', // Sleeper is always head-to-head
      leagueType: leagueType,
      
      // League metadata
      leagueName: league.name || '',
      teamCount: league.total_rosters || settings.num_teams || 0,
      
      // Advanced settings (comprehensive coverage)
      playoffSchedule: settings.playoff_week_start ? `Week ${settings.playoff_week_start}` : null,
      tradeDeadline: tradeDeadlineDate,
      faab: settings.waiver_type === 2, // 2 = FAAB
      
      // Dynasty/Keeper specific settings
      ...(leagueType !== 'Redraft' && {
        maxKeepers: settings.max_keepers,
        taxiSlots: settings.taxi_slots,
        taxiYears: settings.taxi_years,
        reserveSlots: settings.reserve_slots
      }),
    ...
    // Track which fields were synced from API (comprehensive list)
    const syncedFields = [
      'sport', 'scoring', 'matchup', 'leagueType', 'leagueName', 'teamCount'
    ];
    
    // Add conditional synced fields
    if (settings.playoff_week_start) syncedFields.push('playoffSchedule');
    if (settings.trade_deadline) syncedFields.push('tradeDeadline');
    if (settings.waiver_type === 2) syncedFields.push('faab');
    if (leagueType !== 'Redraft') {
      if (settings.max_keepers) syncedFields.push('maxKeepers');
      if (settings.taxi_slots) syncedFields.push('taxiSlots');
      if (settings.reserve_slots) syncedFields.push('reserveSlots');
    }
